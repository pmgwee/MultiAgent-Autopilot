...
| `scripts/autopilot/glm-run.mjs` | 两个模型之间的桥:从 cc-switch **按进程**读取 GLM env(绝不翻转你的全局切换)、prompt 走 **stdin**(Windows 安全)、**应答模型不是 GLM 就 exit 3 大声报错**(`MODEL_VERIFIED`)、用 `--loop N` 连续串接最多 N 个 brief —— **一旦某次运行没有推进 `NEXT:` 就立刻停链**(`LOOP_STALLED`)—— 并自带 `--dry-run`(零成本验线)、`--json`(机器可读结果)、每次运行的 **token 台账**(`--usage` 按 brief 汇总)和自己的单元测试。 |
...
## 📊 Token 可见性 —— 这个 phase 到底烧了多少?

**GLM 侧(执行者)—— 自动、精确记录。** 每次运行都往 `handoff/logs/usage.jsonl` 追加一行 JSON(输入/输出 token、cache 读/写、模型 id、brief 编号、时长、runner 版本),并打印一行 `TOKENS in=… out=… cache_read=… cache_write=…`。零配置。想看整个 phase 的合计,在项目根目录:

```bash
node scripts/autopilot/glm-run.mjs --usage      # 按 brief 汇总 + 总计(加 --json 输出 USAGE_JSON=)
```

autopilot 的 phase 收尾报告也会带同样的合计。一个注意点:台账里的 `cost_usd` 是 CLI 按 **Anthropic** 价格表折算的名义数字 —— 走 GLM 订阅时它**不是**你的真实账单。看 token,别看美元。

**Desktop 侧(orchestrator)—— 你的 Claude 订阅。** session 无法测量自己的 token,管线不做猜测;要精确数字,用真正知道答案的工具:

- Claude Code desktop → `/usage` —— 你套餐的实时用量;或
- `npx ccusage` —— 读取本地 `~/.claude/projects` 会话记录,按天/模型/session 给出精确 token 数。
