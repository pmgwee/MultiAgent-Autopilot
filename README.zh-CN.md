...
# 🤖 Multiagent Automation — Claude Code 的自动驾驶 phase 管线
...
改编自 [pinjun99/Sildenafil_coding](https://github.com/pinjun99/Sildenafil_coding)(orchestrator · advisor · handoff,MIT)—— 把三个模式融合成一条管线,接在"双订阅"机器上(Anthropic 顶级模型 + cc-switch 里的副模型)。
...
**前置要求:** Claude Code ≥ 2.x(desktop + CLI)· Node ≥ 22.5 · [cc-switch](https://github.com/farion1231/cc-switch) 且已配置你的副模型 provider · git。
...
| `scripts/autopilot/glm-run.mjs` | 两个模型之间的桥:从 cc-switch **按进程**读取副模型 env(绝不翻转你的全局切换)、prompt 走 **stdin**(Windows 安全)、**应答模型不对就 exit 3 大声报错**(`MODEL_VERIFIED`)、用 `--loop N` 自己连续串接最多 N 个 brief。 |
...
- [ ] **1. 拿到代码** — `git clone https://github.com/pmgwee/multiagent-automation`,或点 **Code → Download ZIP** 解压。
...
cp multiagent-automation/scripts/autopilot/glm-run.mjs <你的项目>/scripts/autopilot/
```

```powershell
# Windows PowerShell:
New-Item -ItemType Directory -Force <你的项目>\scripts\autopilot
Copy-Item multiagent-automation\scripts\autopilot\glm-run.mjs <你的项目>\scripts\autopilot\
```
...
> 💡 用别的副模型?`--provider "<正则>"` 可按 name/URL 选中任意 cc-switch 条目。
...
- 其他 cc-switch provider(Kimi、Qwen……):`--provider "<正则>"` 已能选中,但当前的 `MODEL_VERIFIED` 门槛只认 GLM/Zhipu 的模型 id —— 其他 provider 需要改这一行检查(已列入待办;目前 GLM 优先)。
- skills 是纯 markdown:改它,它就是你的。项目专属规则放进各项目的 `CLAUDE.md`(执行者协议把那个文件当硬约束)。

## License

MIT —— 见 [LICENSE](LICENSE)。部分改编自 [pinjun99/Sildenafil_coding](https://github.com/pinjun99/Sildenafil_coding)(MIT)。
